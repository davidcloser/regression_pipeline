...
correlation = SelectKBest(score_func=f_regression, k="all").fit(data.drop(label, axis=1), data[label])
...
correlation_df.to_csv("output/correlation_df.csv")
...
models = {
    "Lasso": (Lasso, {"alpha": np.arange(0.5, 2.5, 0.5)}),
    "AdaBoost": (AdaBoostRegressor, {"learning_rate": np.arange(0.5, 2, 0.5), "loss": ["linear", "square", "exponential"]}),
    "RandomForest": (RandomForestRegressor, {"n_estimators": np.arange(10, 100, 10,)}),
